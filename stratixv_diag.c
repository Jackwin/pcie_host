/************************************************************************
*  File: stratixv_diag.c
*
*  Sample user-mode diagnostics application for accessing STRATIXV
...
/* --------------------------------------------------
    STRATIXV configuration registers information
   -------------------------------------------------- */
/* Configuration registers information array */
const WDC_REG gSTRATIXV_CfgRegs[] = {
...
#define STRATIXV_CFG_REGS_NUM sizeof(gSTRATIXV_CfgRegs) / sizeof(WDC_REG)
/* TODO: For read-only or write-only registers, change the direction field of
         the relevant registers in gSTRATIXV_CfgRegs to WDC_READ or WDC_WRITE. */
/* NOTE: You can define additional configuration registers in gSTRATIXV_CfgRegs. */
const WDC_REG *gpSTRATIXV_CfgRegs = gSTRATIXV_CfgRegs;

/* -----------------------------------------------
    STRATIXV run-time registers information
   ----------------------------------------------- */
/* Run-time registers information array */
/* const WDC_REG gSTRATIXV_Regs[]; */
const WDC_REG *gpSTRATIXV_Regs = NULL;
/* TODO: You can remove the comment from the gSTRATIXV_Regs array declaration and
         fill the array with run-time registers information for your device,
         in which case be sure to set gpSTRATIXV_Regs to point to gSTRATIXV_Regs. */
#define STRATIXV_REGS_NUM 0
...
static void DiagIntHandler(WDC_DEVICE_HANDLE hDev, STRATIXV_INT_RESULT *pIntResult);
...
int main(void)
{
    ...
    printf("\n");
    printf("STRATIXV diagnostic utility.\n");
    printf("Application accesses hardware using " WD_PROD_NAME "\n"
        "and a Kernel PlugIn driver (%s).\n", KP_STRATIXV_DRIVER_NAME);
    ...
    dwStatus = STRATIXV_LibUninit();
    if (WD_STATUS_SUCCESS != dwStatus)
        STRATIXV_ERR("stratixv_diag: Failed to uninit the STRATIXV library: %s", STRATIXV_GetLastErr());
...
static void MenuMain(WDC_DEVICE_HANDLE *phDev)
{
    ...
    {
        printf("\n");
        printf("STRATIXV main menu\n");
        printf("--------------\n");
        printf("%d. Scan PCI bus\n", MENU_MAIN_SCAN_PCI_BUS);
        printf("%d. Find and open a STRATIXV device\n", MENU_MAIN_FIND_AND_OPEN);
        if (*phDev)
        {
        ...
        case MENU_MAIN_SCAN_PCI_BUS: /* Scan PCI bus */
            ...
            break;
        case MENU_MAIN_FIND_AND_OPEN: /* Find and open a STRATIXV device */
            if (*phDev)
                DeviceClose(*phDev);
        ...
        case MENU_MAIN_RW_ADDR: /* Read/write memory and I/O addresses */
            ...
            break;
        case MENU_MAIN_RW_CFG_SPACE: /* Read/Write the STRATIXV configuration space */
...
/* -----------------------------------------------
    Device find, open and close
   ----------------------------------------------- */
/* Find and open a STRATIXV device */
static WDC_DEVICE_HANDLE DeviceFindAndOpen(DWORD dwVendorId, DWORD dwDeviceId)
{
    WD_PCI_SLOT slot;
...
static BOOL DeviceFind(DWORD dwVendorId, DWORD dwDeviceId, WD_PCI_SLOT *pSlot)
{
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        STRATIXV_ERR("DeviceFind: Failed scanning the PCI bus.\n"
    ...
    if (!dwNumDevices)
    {
        STRATIXV_ERR("No matching device was found for search criteria "
...
/* Open a handle to a STRATIXV device */
static WDC_DEVICE_HANDLE DeviceOpen(const WD_PCI_SLOT *pSlot)
{
    WDC_DEVICE_HANDLE hDev;
    DWORD dwStatus, dwKPResult = 0;
    WD_PCI_CARD_INFO deviceInfo;
    KP_STRATIXV_VERSION kpVer;
    ...
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        STRATIXV_ERR("DeviceOpen: Failed retrieving the device's resources information.\n"
            "Error 0x%lx - %s\n", dwStatus, Stat2Str(dwStatus));
        return NULL;
    }
    ...
    hDev = STRATIXV_DeviceOpen(&deviceInfo);
    if (!hDev)
    {
        STRATIXV_ERR("DeviceOpen: Failed opening a handle to the device: %s",
            STRATIXV_GetLastErr());
        return NULL;
    }
    ...
    dwStatus = WDC_CallKerPlug(hDev, KP_STRATIXV_MSG_VERSION, &kpVer, &dwKPResult);

    printf("\n");
    if (WD_STATUS_SUCCESS != dwStatus)
    {
        STRATIXV_ERR("Failed sending a 'Get Version' message (0x%x) to the Kernel PlugIn driver (%s).\n"
            "Error 0x%lx - %s\n",
            KP_STRATIXV_MSG_VERSION, KP_STRATIXV_DRIVER_NAME, dwStatus, Stat2Str(dwStatus));
    }
    else if (KP_STRATIXV_STATUS_OK != dwKPResult)
    {
        STRATIXV_ERR("Kernel PlugIn 'Get Version' message (0x%x) failed. "
"Kernel PlugIn status: 0x%lx\n", KP_STRATIXV_MSG_VERSION, dwKPResult);
    }
    else
    {
        printf("Using %s Kernel-Plugin driver version %ld.%02ld - %s\n",
            KP_STRATIXV_DRIVER_NAME, kpVer.dwVer/100, kpVer.dwVer%100, kpVer.cVer);
    }
    return hDev;
}

/* Close handle to a STRATIXV device */
static void DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    if (!hDev)
        return;

    if (!STRATIXV_DeviceClose(hDev))
    {
        STRATIXV_ERR("DeviceClose: Failed closing STRATIXV device: %s",
            STRATIXV_GetLastErr());
    }
}
...
static void MenuReadWriteAddr(WDC_DEVICE_HANDLE hDev)
{
    ...
    {
        DWORD dwNumAddrSpaces = STRATIXV_GetNumAddrSpaces(hDev);
        ...
        {
            STRATIXV_ERR("MenuReadWriteAddr: Error - no active address spaces found\n");
...
static void SetAddrSpace(WDC_DEVICE_HANDLE hDev, PDWORD pdwAddrSpace)
{
    DWORD dwAddrSpace;
    DWORD dwNumAddrSpaces = STRATIXV_GetNumAddrSpaces(hDev);
    STRATIXV_ADDR_SPACE_INFO addrSpaceInfo;
    ...
    for (dwAddrSpace = 0; dwAddrSpace < dwNumAddrSpaces; dwAddrSpace++)
    {
        ...
        if (!STRATIXV_GetAddrSpaceInfo(hDev, &addrSpaceInfo))
        {
            STRATIXV_ERR("SetAddrSpace: Error - Failed to get address space information: %s",
                STRATIXV_GetLastErr());
            return;
        }
...
static void MenuReadWriteCfgSpace(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        ...
        if (STRATIXV_CFG_REGS_NUM)
        {
            printf("\n");
            printf("Configuration registers:\n");
            printf("------------------------\n");
            WDC_DIAG_RegsInfoPrint(gpSTRATIXV_CfgRegs, STRATIXV_CFG_REGS_NUM,
        ...
        if (DIAG_INPUT_FAIL == DIAG_GetMenuOption(&option,
            STRATIXV_CFG_REGS_NUM ? MENU_RW_CFG_SPACE_WRITE_REG :
            MENU_RW_CFG_SPACE_WRITE_OFFSET))
        {
            continue;
        }
        ...
        case MENU_RW_CFG_SPACE_READ_ALL_REGS:
            WDC_DIAG_ReadRegsAll(hDev, gpSTRATIXV_CfgRegs, STRATIXV_CFG_REGS_NUM, TRUE);
            break;
        case MENU_RW_CFG_SPACE_READ_REG:  /* Read from a configuration register */
            WDC_DIAG_ReadWriteReg(hDev, gpSTRATIXV_CfgRegs, STRATIXV_CFG_REGS_NUM, WDC_READ, TRUE);
            break;
        case MENU_RW_CFG_SPACE_WRITE_REG: /* Write to a configuration register */
            WDC_DIAG_ReadWriteReg(hDev, gpSTRATIXV_CfgRegs, STRATIXV_CFG_REGS_NUM, WDC_WRITE, TRUE);
            break;
        }
...
static void MenuReadWriteRegs(WDC_DEVICE_HANDLE hDev)
{
    ...
    do {
        /* Display pre-defined registers' information */
        printf("\n");
        printf("STRATIXV run-time registers:\n");
        printf("--------------------------\n");   
        WDC_DIAG_RegsInfoPrint(gpSTRATIXV_Regs, STRATIXV_REGS_NUM, WDC_DIAG_REG_PRINT_ALL);

        printf("\n");
        printf("Read/write the STRATIXV run-time registers\n");
        printf("-----------------------------------------\n");
        ...
        case MENU_RW_REGS_READ_ALL:
            WDC_DIAG_ReadRegsAll(hDev, gpSTRATIXV_Regs, STRATIXV_REGS_NUM, FALSE);
            break;
        case MENU_RW_REGS_READ_REG:  /* Read from a register */
            WDC_DIAG_ReadWriteReg(hDev, gpSTRATIXV_Regs, STRATIXV_REGS_NUM, WDC_READ, FALSE);
            break;
        case MENU_RW_REGS_WRITE_REG: /* Write to a register */
            WDC_DIAG_ReadWriteReg(hDev, gpSTRATIXV_Regs, STRATIXV_REGS_NUM, WDC_WRITE, FALSE);
            break;
        }
...
static void MenuInterrupts(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        fIntEnable = !STRATIXV_IntIsEnabled(hDev);
        ...
        case MENU_INT_ENABLE_DISABLE: /* Enable/disable interrupts */
            if (fIntEnable)
            {
                DWORD dwStatus = STRATIXV_IntEnable(hDev, DiagIntHandler);

                if (WD_STATUS_SUCCESS == dwStatus)
                    printf("Interrupts enabled\n");
                else
                {
                    STRATIXV_ERR("Failed enabling interrupts. Error 0x%lx - %s\n",
                       dwStatus, Stat2Str(dwStatus));
                }
            }
            else
            {
                if (WD_STATUS_SUCCESS == STRATIXV_IntDisable(hDev))
                    printf("Interrupts disabled\n");
                else
                    STRATIXV_ERR("Failed disabling interrupts: %s", STRATIXV_GetLastErr());
            }
            break;
        }
...
static void MenuEvents(WDC_DEVICE_HANDLE hDev)
{
    ...
    do
    {
        fRegister = !STRATIXV_EventIsRegistered(hDev);
        ...
        case MENU_EVENTS_REGISTER_UNREGISTER: /* Register/unregister events */
            if (fRegister)
            {
                if (WD_STATUS_SUCCESS == STRATIXV_EventRegister(hDev, DiagEventHandler))
                    printf("Events registered\n");
                else
                    STRATIXV_ERR("Failed to register events. Last error:\n%s", STRATIXV_GetLastErr());
            }
            else
            {
                if (WD_STATUS_SUCCESS == STRATIXV_EventUnregister(hDev))
                    printf("Events unregistered\n");
                else
                    STRATIXV_ERR("Failed to unregister events. Last Error:\n%s", STRATIXV_GetLastErr());
            }
            break;
        }
