...
*  File: stratixv_lib.c
*
*  Library for accessing STRATIXV devices using a Kernel PlugIn driver.
...
#define STRATIXV_DEFAULT_LICENSE_STRING "6C3CC2CFE89E7AD0424A070D434A6F6DC49520ED.wps"
#define STRATIXV_DEFAULT_DRIVER_NAME WD_DEFAULT_DRIVER_NAME_BASE

/* STRATIXV device information struct */
typedef struct {
    STRATIXV_INT_HANDLER   funcDiagIntHandler;
    STRATIXV_EVENT_HANDLER funcDiagEventHandler;
} STRATIXV_DEV_CTX, *PSTRATIXV_DEV_CTX;
/* TODO: You can add fields to store additional device-specific information */

static CHAR gsSTRATIXV_LastErr[256];
...
static void DLLCALLCONV STRATIXV_IntHandler(PVOID pData);
static void STRATIXV_EventHandler(WD_EVENT *pEvent, PVOID pData);
#endif
...
static inline BOOL IsValidDevice(PWDC_DEVICE pDev, const CHAR *sFunc)
{
    ...
    {
        snprintf(gsSTRATIXV_LastErr, sizeof(gsSTRATIXV_LastErr) - 1, "%s: NULL device %s\n",
            sFunc, !pDev ? "handle" : "context");
        ErrLog(gsSTRATIXV_LastErr);
        return FALSE;
    }
 ...
 *************************************************************/
/* -----------------------------------------------
    STRATIXV and WDC library initialize/uninit
   ----------------------------------------------- */
DWORD STRATIXV_LibInit(void)
{
    DWORD dwStatus;
    ...
    /* Set the driver name */
    if (!WD_DriverName(STRATIXV_DEFAULT_DRIVER_NAME))
    {
    ...
    dwStatus = WDC_DriverOpen(WDC_DRV_OPEN_DEFAULT, STRATIXV_DEFAULT_LICENSE_STRING);
    if (WD_STATUS_SUCCESS != dwStatus)
    {
...
/* -----------------------------------------------
    Device open/close
   ----------------------------------------------- */
WDC_DEVICE_HANDLE STRATIXV_DeviceOpen(const WD_PCI_CARD_INFO *pDeviceInfo)
{
    DWORD dwStatus;
    PSTRATIXV_DEV_CTX pDevCtx = NULL;
    WDC_DEVICE_HANDLE hDev = NULL;

    /* Validate arguments */
    if (!pDeviceInfo)
    {
        ErrLog("STRATIXV_DeviceOpen: Error - NULL device information struct pointer\n");
        return NULL;
    }

    /* Allocate memory for the STRATIXV device context */
    pDevCtx = (PSTRATIXV_DEV_CTX)malloc(sizeof (STRATIXV_DEV_CTX));
    if (!pDevCtx)
    {
        ErrLog("Failed allocating memory for STRATIXV device context\n");
        return NULL;
    }
    ...
    dwStatus = WDC_PciDeviceOpen(&hDev, pDeviceInfo, pDevCtx, NULL, KP_STRATIXV_DRIVER_NAME, &hDev);
    ...
    TraceLog("STRATIXV_DeviceOpen: Opened a STRATIXV device (handle 0x%p)\n"        
"Device uses a Kernel PlugIn driver (%s)\n", hDev, KP_STRATIXV_DRIVER_NAME);
    return hDev;

Error:    
    if (hDev)
        STRATIXV_DeviceClose(hDev);
    else
        free(pDevCtx);
...
BOOL STRATIXV_DeviceClose(WDC_DEVICE_HANDLE hDev)
{
    DWORD dwStatus;
    PWDC_DEVICE pDev = (PWDC_DEVICE)hDev;
    PSTRATIXV_DEV_CTX pDevCtx;
    
    TraceLog("STRATIXV_DeviceClose entered. Device handle: 0x%p\n", hDev);

    if (!hDev)
    {
        ErrLog("STRATIXV_DeviceClose: Error - NULL device handle\n");
        return FALSE;
    }

    pDevCtx = (PSTRATIXV_DEV_CTX)WDC_GetDevContext(pDev);
    
    /* Disable interrupts */
    if (WDC_IntIsEnabled(hDev))
    {
        dwStatus = STRATIXV_IntDisable(hDev);
        if (WD_STATUS_SUCCESS != dwStatus)
        {
    ...
    /* Free STRATIXV device context memory */
    if (pDevCtx)
        free (pDevCtx);
...
static void DLLCALLCONV STRATIXV_IntHandler(PVOID pData)
{
    ...
    PSTRATIXV_DEV_CTX pDevCtx = (PSTRATIXV_DEV_CTX)WDC_GetDevContext(pDev);
    STRATIXV_INT_RESULT intResult;
...
DWORD STRATIXV_IntEnable(WDC_DEVICE_HANDLE hDev, STRATIXV_INT_HANDLER funcIntHandler)
{
    DWORD dwStatus;
    PWDC_DEVICE pDev = (PWDC_DEVICE)hDev;
    PSTRATIXV_DEV_CTX pDevCtx;

    TraceLog("STRATIXV_IntEnable entered. Device handle: 0x%p\n", hDev);

    if (!IsValidDevice(pDev, "STRATIXV_IntEnable"))
    ...
    /* Store the diag interrupt handler routine, which will be executed by
       STRATIXV_IntHandler() when an interrupt is received */
    ...
    dwStatus = WDC_IntEnable(hDev, NULL, 0, 0,
        STRATIXV_IntHandler, (PVOID)pDev, WDC_IS_KP(hDev));
    ...
    TraceLog("STRATIXV_IntEnable: Interrupts enabled\n");

    return WD_STATUS_SUCCESS;
}

DWORD STRATIXV_IntDisable(WDC_DEVICE_HANDLE hDev)
{
    DWORD dwStatus;
    PWDC_DEVICE pDev = (PWDC_DEVICE)hDev;
    PSTRATIXV_DEV_CTX pDevCtx;

    TraceLog("STRATIXV_IntDisable entered. Device handle: 0x%p\n", hDev);

    if (!IsValidDevice(pDev, "STRATIXV_IntDisable"))
        return WD_INVALID_PARAMETER;
...
BOOL STRATIXV_IntIsEnabled(WDC_DEVICE_HANDLE hDev)
{
    if (!IsValidDevice((PWDC_DEVICE)hDev, "STRATIXV_IntIsEnabled"))
        return FALSE;
...
static void STRATIXV_EventHandler(WD_EVENT *pEvent, PVOID pData)
{
    ...
    TraceLog("STRATIXV_EventHandler entered, pData: 0x%p, dwAction 0x%lx\n",
...
DWORD STRATIXV_EventRegister(WDC_DEVICE_HANDLE hDev, STRATIXV_EVENT_HANDLER funcEventHandler)
{
    DWORD dwStatus;
    PWDC_DEVICE pDev = (PWDC_DEVICE)hDev;
    PSTRATIXV_DEV_CTX pDevCtx;
    ...
    TraceLog("STRATIXV_EventRegister entered. Device handle: 0x%p\n", hDev);
    
    if (!IsValidDevice(pDev, "STRATIXV_EventRegister"))
        return WD_INVALID_PARAMETER;
    ...
    /* Store the diag event handler routine to be executed from STRATIXV_EventHandler() upon an event */
    pDevCtx->funcDiagEventHandler = funcEventHandler;

    /* Register event */
    dwStatus = WDC_EventRegister(hDev, dwActions, STRATIXV_EventHandler, hDev, TRUE);
...
DWORD STRATIXV_EventUnregister(WDC_DEVICE_HANDLE hDev)
{
    DWORD dwStatus;
    
    TraceLog("STRATIXV_EventUnregister entered. Device handle: 0x%p\n", hDev);
    
    if (!IsValidDevice((PWDC_DEVICE)hDev, "STRATIXV_EventUnregister"))
        return WD_INVALID_PARAMETER;
...
BOOL STRATIXV_EventIsRegistered(WDC_DEVICE_HANDLE hDev)
{
    if (!IsValidDevice((PWDC_DEVICE)hDev, "STRATIXV_EventIsRegistered"))
        return FALSE;
...
/* -----------------------------------------------
   ...
   ----------------------------------------------- */
DWORD STRATIXV_GetNumAddrSpaces(WDC_DEVICE_HANDLE hDev)
{
    PWDC_DEVICE pDev = (PWDC_DEVICE)hDev;
    
    if (!IsValidDevice(pDev, "STRATIXV_GetNumAddrSpaces"))
        return 0;

    return pDev->dwNumAddrSpaces;
}

BOOL STRATIXV_GetAddrSpaceInfo(WDC_DEVICE_HANDLE hDev, STRATIXV_ADDR_SPACE_INFO *pAddrSpaceInfo)
{
    PWDC_DEVICE pDev = (PWDC_DEVICE)hDev;
    ...
    if (!IsValidDevice(pDev, "STRATIXV_GetAddrSpaceInfo"))
        return FALSE;

#if defined(DEBUG)
    if (!pAddrSpaceInfo)
    {
        ErrLog("STRATIXV_GetAddrSpaceInfo: Error - NULL address space information pointer\n");
        return FALSE;
    }
#endif
    ...
    {
        ErrLog("STRATIXV_GetAddrSpaceInfo: Error - Address space %ld is out of range (0 - %ld)\n",
...
static void ErrLog(const CHAR *sFormat, ...)
{
    ...
    vsnprintf(gsSTRATIXV_LastErr, sizeof(gsSTRATIXV_LastErr) - 1, sFormat, argp);
#if defined(DEBUG)
#if defined (__KERNEL__)
    WDC_Err("KP STRATIXV lib: %s", STRATIXV_GetLastErr());
#else
    WDC_Err("STRATIXV lib: %s", STRATIXV_GetLastErr());
#endif
#endif
    va_end(argp);
}
...
#if defined (__KERNEL__)
    WDC_Trace("KP STRATIXV lib: %s", sMsg);
#else
    WDC_Trace("STRATIXV lib: %s", sMsg);
#endif
    va_end(argp);
#endif
}

const char *STRATIXV_GetLastErr(void)
{
    return gsSTRATIXV_LastErr;
}
